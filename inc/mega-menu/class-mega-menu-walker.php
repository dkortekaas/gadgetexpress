...
class Toffe_Dassen_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_is_mega         = apply_filters( 'gadgetexpress_menu_item_mega', get_post_meta( $item->ID, 'tamm_menu_item_mega', true ), $item->ID );
		...
		if ( $item_hot || $item_new || $item_trending ) {
			...
			if ( $item_hot ) {
				$badge[] = '<span class="hot-badge">' . esc_html__( 'Hot', 'gadget' ) . '</span>';
			}
			if ( $item_new ) {
				$badge[] = '<span class="new-badge">' . esc_html__( 'New', 'gadget' ) . '</span>';
			}
			if ( $item_trending ) {
				$badge[] = '<span class="trending-badge">' . esc_html__( 'Trending', 'gadget' ) . '</span>';
			}
			$badge[] = '</span>';
		}
