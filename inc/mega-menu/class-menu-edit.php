...
class Toffe_Dassen_Mega_Menu_Walker_Edit extends Walker_Nav_Menu_Edit {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( $cancel ) {
			...
			$link->nodeValue = esc_html__( 'Settings', 'gadget' );
...
class Toffe_Dassen_Mega_Menu_Edit {
	/**
	 * Gadget Express_Mega_Menu_Edit constructor.
	 */
	...
	public function scripts( $hook ) {
		...
		wp_register_style( 'gadgetexpress-mega-menu', get_template_directory_uri() . '/inc/mega-menu/css/mega-menu.css', array( 'media-views', 'wp-color-picker' ), '20160530' );
		wp_enqueue_style( 'gadgetexpress-mega-menu' );

		wp_register_script( 'gadgetexpress-mega-menu', get_template_directory_uri() . '/inc/mega-menu/js/mega-menu.js', array( 'jquery', 'jquery-ui-resizable', 'backbone', 'underscore', 'wp-color-picker' ), '20160530', true );
		wp_enqueue_media();
		wp_enqueue_script( 'gadgetexpress-mega-menu' );
	}
		...
		<div id="tamm-settings" tabindex="0" class="tamm-settings">
			<div class="tamm-modal media-modal wp-core-ui">
				<button type="button" class="button-link media-modal-close tamm-modal-close">
					<span class="media-modal-icon"><span class="screen-reader-text"><?php esc_html_e( 'Close', 'gadget' ) ?></span></span>
				</button>
					...
					<div class="tamm-frame-toolbar media-frame-toolbar">
						<div class="tamm-toolbar media-toolbar">
							<div class="tamm-toolbar-primary media-toolbar-primary search-form">
								<button type="button" class="button tamm-button tamm-button-save media-button button-primary button-large"><?php esc_html_e( 'Save Changes', 'gadget' ) ?></button>
								<button type="button" class="button tamm-button tamm-button-cancel media-button button-secondary button-large"><?php esc_html_e( 'Cancel', 'gadget' ) ?></button>
								<span class="spinner"></span>
							</div>
						</div>
					</div>
