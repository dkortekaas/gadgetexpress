 ...
 */
function gadgetexpress_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'gadget' ),
			'archive'   => esc_html__( 'Archives', 'gadget' ),
			'blog'      => esc_html__( 'Blog', 'gadget' ),
			'search'    => esc_html__( 'Search results for', 'gadget' ),
			'not_found' => esc_html__( 'Not Found', 'gadget' ),
			'author'    => esc_html__( 'Author Archives:', 'gadget' ),
			'day'       => esc_html__( 'Daily Archives:', 'gadget' ),
			'month'     => esc_html__( 'Monthly Archives:', 'gadget' ),
			'year'      => esc_html__( 'Yearly Archives:', 'gadget' ),
		),
	) );

	$args = apply_filters( 'gadgetexpress_breadcrumbs_args', $args );
	...
	elseif ( is_single() ) {
		...
		if ( is_singular( 'product' ) ) {
			...
			if ( $page_id = get_option( 'gadgetexpress_portfolio_page_id' ) ) {
				$items[] = sprintf( $item_tpl, esc_url( get_permalink( $page_id ) ), get_the_title( $page_id ) );
			}
		} elseif ( is_singular( 'service' ) ) {
			$taxonomy = 'service_category';

			if ( $page_id = get_option( 'gadgetexpress_service_page_id' ) ) {
				$items[] = sprintf( $item_tpl, esc_url( get_permalink( $page_id ) ), get_the_title( $page_id ) );
			}
		}
		...
		if ( $terms ) {
			$term    = current( $terms );
			$terms   = gadgetexpress_get_term_parents( $term->term_id, $taxonomy );
			$terms[] = $term->term_id;
	...
	elseif ( is_page() ) {
		if ( ( function_exists( 'is_cart' ) && is_cart() ) || ( function_exists( 'is_checkout' ) && is_checkout() ) ) {
			...
			$pages = gadgetexpress_get_post_parents( get_queried_object_id() );
	...
	elseif ( is_tax() || is_category() || is_tag() ) {
		...
		if ( $current_term ) {
			$terms = gadgetexpress_get_term_parents( get_queried_object_id(), $current_term->taxonomy );
			if ( $terms ) {
				foreach ( $terms as $term_id ) {
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'gadget' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'gadget' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'gadget' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	elseif ( is_post_type_archive( 'portfolio' ) ) {

		$portfolio_page_id = get_option( 'gadgetexpress_portfolio_page_id' );

		if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
			$title = get_the_title( $portfolio_page_id );
		} else {
			$title = _x( 'Portfolio', 'Portfolio post type breadcrumb', 'gadget' );
		}
 ...
 */
function gadgetexpress_get_term_parents( $term_id = '', $taxonomy = 'category' ) {
 ...
 */
function gadgetexpress_get_post_parents( $post_id = '' ) {
